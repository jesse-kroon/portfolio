<script>
	import Section from '../../components/Projects/Section/Section.svelte';
</script>

<h1 class="mb-4 text-2xl text-white">My Projects</h1>
<p class="text-white">
	Below you can find both projects I have completed and ones I am working on. These projects will be
	divided in the following sections:
</p>
<ul class="my-4 ml-6 list-disc text-sm text-white">
	<li>Web Applications</li>
	<li>API's</li>
	<li>Frontend Mentor components</li>
</ul>

<Section title="Web Applications" />
<Section title="API's" />
<Section title="Frontend Mentor components" />
